...
def delete(id):
    ...
    db.engine.execute(f"DELETE FROM `marks` WHERE `marks`.`id`={id}")
...
def addmarks(rollno):
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"INSERT INTO `marks` (`rollno`,`mid_1`,`mid_2`,`final`) VALUES ('{roll}','{mid1}','{mid2}','{final}')")
